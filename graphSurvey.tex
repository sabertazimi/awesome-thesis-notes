...
Previous surveys included many famous systems~\cite{DBLP:journals/csur/McCuneWM15}.
...
$C_i$ represents destination of i-nth outgoing edge.
For bits of $R_i$:
\begin{compactitem}
  \item $R_i[0]$ represent visit status.
  \item $R_i[1]$ represnet existence of outgoing edge of $Node_i$.
  \item $R_i[63...2]$:
  when $R_i[0] == 0$ (not visited),
  $R_i[63...32]$ for neighbors' start offset
  and $R_i[31...2]$ for neighbors' end offset
  ($for (cur= R_i[63...32], end = R_i[31...2]; cur < end; cur += 1) \{ C_{cur}... \}$);
  when $R_i[0] == 1$ (visited),
  $R_i[63...1]$ for visit level of $Node_i$.
\end{compactitem}
